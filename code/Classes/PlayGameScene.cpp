...
void PlayGameScene::addMonsters() {
	//根据关卡设置事件
	GlobalVar::getInstance()->level++;
	GlobalVar::getInstance()->time = 140 - 20 * GlobalVar::getInstance()->level;
	GlobalVar::getInstance()->count = 0;
	// 通关音效
	if(GlobalVar::getInstance()->level!=1) CocosDenshion::SimpleAudioEngine::getInstance()->playEffect("music/victory.mp3", false);
	//通过getObjectGroup获得对象层
	TMXObjectGroup *objects = tileMap->getObjectGroup("Object");
	int count = 0;
...
{	
	...
	//关卡时间显示
	Level = Label::createWithTTF("Level: ", "fonts/Marker Felt.ttf", 24);
	Level->setPosition(actualPosition + Vec2(0, visibleSize.height / 2 - 10));
	this->addChild(Level);
	Time = Label::createWithTTF("Time: ", "fonts/Marker Felt.ttf", 24);
	Time->setPosition(Level->getPosition() - Vec2(0, 30));
	this->addChild(Time);
...
void PlayGameScene::update(float f) {	
	...
	str = "Level: " + Value(GlobalVar::getInstance()->level).asString();
	Level->setString(str);
	str = "Time: " + Value(GlobalVar::getInstance()->time).asString();
	Time->setString(str);
	...
	//关卡设置
	GlobalVar::getInstance()->count++;
	if (GlobalVar::getInstance()->count == 20) {
		GlobalVar::getInstance()->time--;
		GlobalVar::getInstance()->count = 0;
	}
	if (time == 0) gameOver();
}
